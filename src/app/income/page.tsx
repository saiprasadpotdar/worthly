    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-neutral-500 text-sm mt-1">Track your annual income and growth</p>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Card>
              <CardContent className="p-5">
                <p className="text-sm text-neutral-500 mb-1">Latest Net Income</p>
                <p className="text-xl font-bold">{latest ? formatCurrency(latest.netIncome, true) : '—'}</p>
                <p className="text-xs text-neutral-400 mt-1">{latest?.year}</p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-5">
                <p className="text-sm text-neutral-500 mb-1">YoY Growth</p>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={chartData}>
                  ...
                  <Bar dataKey="net" fill="#171717" radius={[4, 4, 0, 0]} name="Net Income" />
                  <Bar dataKey="taxes" fill="#d4d4d4" radius={[4, 4, 0, 0]} name="Taxes" />
                </BarChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-neutral-100">
                      <th className="text-left py-2 font-medium text-neutral-500">Year</th>
                      <th className="text-right py-2 font-medium text-neutral-500">Gross Salary</th>
                      <th className="text-right py-2 font-medium text-neutral-500">Taxes</th>
                      <th className="text-right py-2 font-medium text-neutral-500">Net Income</th>
                      <th className="text-right py-2 font-medium text-neutral-500">Growth</th>
                    ...
                    {incomes.map((item, idx) => {
                      ...
                      return (
                        <tr key={item.id} className="border-b border-neutral-50 hover:bg-neutral-50/50">
                          ...
                          <td className="py-2.5 text-right text-neutral-500">{formatCurrency(item.taxes)}</td>
      ...
      <Dialog open={showForm} onClose={() => setShowForm(false)} title={editing ? 'Edit Income Year' : 'Add Income Year'}>
        <div className="space-y-4">
          ...
          <div className="rounded-lg bg-neutral-50 p-3">
            <p className="text-sm text-neutral-500">Net Income: <span className="font-medium text-neutral-900">{formatCurrency(form.grossSalary - form.taxes)}</span></p>
          </div>
